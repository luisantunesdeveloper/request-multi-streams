...
const req2 = {
  options: {
    url:
      'https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcS2bZdzOUGStsuLVzH79PTGHMoJ0B_ZpUcJylVdveVd4p5oyywvSCRaHSg',
  },
};
...
test('it should emit error', assert => {
  ...
  resultEmitter[req1.options.url].on('error', fn);
  resultEmitter[req1.options.url].emit('error', {
    reqNumber: 1,
    args: req1,
    error: new Error('some error'),
  });
