  ...
  if (emitters) {
    for (var name in emitters) {
      ...
      emitters[name].on('progress', data => {
        console.log(
          chalk.yellow(
            `Request ${data.reqNumber} for ${data.args.options.url}: ${Number(
              (data.progress.percent * 100).toFixed(1)
            )}% complete`
          )
        );
      });
      ...
      emitters[name].on('response', data => {
        ...
        if (name === 'http://www.textfiles.com/fun/acronym.txt') {
          data.stream.pipe(
            fs.createWriteStream(`${__dirname}/${data.reqNumber}`)
          );
        }
      });
      ...
      emitters[name].on('error', error => {
        console.log(
          chalk.red(
            `Request ${error.reqNumber} for ${
              error.args.options.url
            }: finished on error ${error.error}`
          )
        );
      });
